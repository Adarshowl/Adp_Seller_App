...
const PaymentHistory = ({navigation}) => {
  ...
  const getAllShop = () => {
    setLoading(true);
    try {
      // console.log("response axios >>> ", JSON.stringify(API_END_POINTS.PAYMENT_LIST_API));

      ApiCall('get', null, API_END_POINTS.PAYMENT_LIST_API, {
        'Content-Type': 'application/json',
        'x-access-token': userToken || userData?.remember_token,
      })
        .then(response => {
          if (response?.statusCode === 200) {
            console.log('Response payment: ', JSON.stringify(response.data));
      ...
      <ScrollView
        ...
        <View
          ...
          <FlatList
            ...
            data={orderList}
            renderItem={({item, index}) => <PaymentListData item={item} />}
          />
        </View>
      </ScrollView>
    </SafeAreaView>
