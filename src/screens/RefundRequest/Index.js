...
const RefundRequest = ({navigation}) => {
  ...
  const getRefundRequest = () => {
    setLoading(true);
    try {
      // console.log("response axios >>> ", JSON.stringify(API_END_POINTS.REFUND_REQUEST_LIST));
      // console.log("response axios >>> ", JSON.stringify(userToken));

      ApiCall('get', null, API_END_POINTS.REFUND_REQUEST_LIST, {
        ...
        .then(response => {
          ...
          if (response?.statusCode === 200) {
            console.log("Response data: ", JSON.stringify(response.data));
            setRefundRequestList(response?.data?.response);
