...
const Order = ({navigation}) => {
  ...
  const getAllShop = () => {
    setLoading(true);
    try {
      // console.log("response axios >>> ", JSON.stringify(API_END_POINTS.ORDER_LIST));
      // console.log("orderList", userToken)

      ApiCall('get', null, API_END_POINTS.ORDER_LIST, {
      ...
      <ScrollView
        ...
        <View
          ...
          <FlatList
            ...
            data={orderList}
            renderItem={({item, index}) => <OrderList item={item} />}
          />
        </View>
      </ScrollView>
    </SafeAreaView>
